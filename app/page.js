...
function StripeCheckoutForm(props) {
  var stripe = useStripe(), elements = useElements();
  var [loading, setLoading] = useState(false);

  var handlePayment = async function(e) {
    e.preventDefault();
    if (!stripe || !elements) return;
    if (!props.cNm.trim() || !props.cPh.trim() || !props.cEm.trim()) { props.notify("Fill all fields", C.red); return; }
    ...
    fetch('/api/checkout', {
      ...
      body: JSON.stringify({
        ...
        customerInfo: { name: props.cNm, email: props.cEm, phone: props.cPh }
      })
    }).then(r => r.json()).then(d => {
      if (d.error) { props.notify("Error: " + d.error, C.red); setLoading(false); return; }
      if (d.clientSecret) {
        stripe.confirmCardPayment(d.clientSecret).then(result => {
          if (result.paymentIntent?.status === 'succeeded') {
            props.notify("Payment successful! Order: " + d.orderCode, C.cyan);
            props.onSuccess();
          } else if (result.error) {
            props.notify("Payment failed: " + result.error.message, C.red);
          }
          setLoading(false);
        });
      } else {
        props.notify("Order placed! Code: " + d.orderCode, C.cyan);
        props.onSuccess();
        setLoading(false);
      }
  ...
  return <form onSubmit={handlePayment}>
    <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:600,color:C.sub}}>Name *</label><input type="text" value={props.cNm} onChange={e=>props.setCNm(e.target.value)} placeholder="John Doe" style={{width:"100%",padding:10,background:"#F7F6F3",border:"1.5px solid "+C.bd,borderRadius:6,fontSize:12,marginTop:4,boxSizing:"border-box"}}/></div>
    <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:600,color:C.sub}}>Email *</label><input type="email" value={props.cEm} onChange={e=>props.setCEm(e.target.value)} placeholder="test@example.com" style={{width:"100%",padding:10,background:"#F7F6F3",border:"1.5px solid "+C.bd,borderRadius:6,fontSize:12,marginTop:4,boxSizing:"border-box"}}/></div>
    <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:600,color:C.sub}}>Phone *</label><input type="text" value={props.cPh} onChange={e=>props.setCPh(e.target.value)} placeholder="5551234567" style={{width:"100%",padding:10,background:"#F7F6F3",border:"1.5px solid "+C.bd,borderRadius:6,fontSize:12,marginTop:4,boxSizing:"border-box"}}/></div>
    ...
    <div style={{padding:10,background:"#F7F6F3",borderRadius:6,marginBottom:14}}><div style={{fontSize:11}}>Subtotal: ${props.total.toFixed(2)}</div><div style={{fontSize:11,color:C.sub}}>Tax: ${props.tax.toFixed(2)}</div><div style={{fontSize:14,fontWeight:700,color:C.red,marginTop:4}}>Total: ${props.finalTotal.toFixed(2)}</div></div>
...
export default function App(){
  var s=useState;
  var[disc,setDisc]=s(true),[discOk,setDiscOk]=s(false);
  var[cart,setCart]=s([]),[cartOpen,setCartOpen]=s(false),[note,setNote]=s(null);
  var[cNm,setCNm]=s(""),[cPh,setCPh]=s(""),[cEm,setCEm]=s(""),[showCheckout,setShowCheckout]=s(false);
  ...
  var onSuccess=function(){setCart([]);setCNm("");setCPh("");setCEm("");setShowCheckout(false);};
  ...
  return <div style={{minHeight:"100vh",background:C.bg,color:C.tx,fontFamily:"DM Sans,sans-serif"}}>
    ...
    {cartOpen&&<div style={{position:"fixed",top:0,right:0,bottom:0,width:"min(360px,90vw)",background:C.cd,zIndex:2000,borderLeft:"1px solid "+C.bd,padding:20,overflowY:"auto"}}><h3 style={{fontSize:16,fontWeight:700,margin:"0 0 14px"}}>Cart</h3>{cart.map(function(it,i){return <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid "+C.bd}}><div><div style={{fontWeight:600,fontSize:11}}>{it.name}</div><div style={{color:C.red,fontSize:12}}>{"$"+it.price.toFixed(2)}</div></div><button onClick={function(){setCart(cart.filter(function(_,x){return x!==i}))}} style={{background:"#fee",color:C.red,border:"none",padding:"3px 7px",borderRadius:4,fontSize:9,cursor:"pointer"}}>Remove</button></div>})}<div style={{marginTop:14,paddingTop:10,borderTop:"2px solid "+C.tx}}><div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:8}}><span>Total:</span><span style={{fontWeight:700,color:C.red}}>{"$"+finalTotal.toFixed(2)}</span></div><button onClick={function(){setCartOpen(false);setShowCheckout(true)}} style={{width:"100%",padding:12,background:C.red,color:"#fff",border:"none",borderRadius:8,fontWeight:700,cursor:"pointer"}}>Go to Checkout</button></div></div>}
    <main style={{maxWidth:1000,margin:"0 auto",padding:"30px 14px"}}>
      {!showCheckout && <div style={{textAlign:"center",marginBottom:40}}><h1 style={{fontFamily:"Playfair Display,serif",fontSize:32,fontWeight:700,margin:"0 0 10px"}}>The HTV Store</h1><p style={{fontSize:14,color:C.sub,maxWidth:400,margin:"0 auto 24px"}}>Custom t-shirts with HTV designs</p><button onClick={function(){setCart([...cart,{name:"Designed Shirt",price:34.99}]);notify("Added to cart!",C.cyan)}} style={{background:C.red,color:"#fff",border:"none",padding:"12px 24px",borderRadius:8,fontWeight:700,cursor:"pointer",marginRight:10}}>Design Shirt</button></div>}
      {showCheckout && <div style={{maxWidth:500,margin:"0 auto",background:C.cd,borderRadius:10,padding:20,border:"1px solid "+C.bd}}><h3 style={{fontSize:16,fontWeight:700,margin:"0 0 16px"}}>Checkout with Card</h3><Elements stripe={stripePromise}><StripeCheckoutForm cart={cart} total={total} tax={tax} finalTotal={finalTotal} cNm={cNm} setCNm={setCNm} cEm={cEm} setCEm={setCEm} cPh={cPh} setCPh={setCPh} notify={notify} onSuccess={onSuccess}/></Elements><button onClick={function(){setShowCheckout(false)}} style={{marginTop:12,padding:"10px 20px",background:"#f5f5f2",border:"none",borderRadius:6,cursor:"pointer",fontSize:12}}>Back to Cart</button></div>}
    </main>
  </div>;
}
